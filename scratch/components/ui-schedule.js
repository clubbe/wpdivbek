...
export class Schedule extends limit.Component {
    ...
    created() {
        ...
        this.onclick = (event) => {
            ...
            LOG.info('onclick > event = ', event.path.find('<a#schedule.item'));
            if (event.path && event.path[0] && event.path[0].id && event.path[0].id.indexOf && event.path[0].id === 'schedule') {

                let schedule = { job: job.value };
